...
Dependensi: docs/REED_RUNTIME_ARCHITECTURE.md, automation/reed-runtime-spec.yaml, docs/INBOX_ROUTING.md, docs/REED_MEMORY_AND_LEARNING.md
...
Kalau ada konflik:
- runtime lokal/VPS menang atas asumsi lokal / notebook
- arsitektur baru (`REED_RUNTIME_ARCHITECTURE.md`) menang atas blueprint lama.
...
- Ikuti Read Order di `startup-template.md`.
...
- approvals tetap dianggap aktif untuk aksi sensitif
<!-- GENERATED:CURRENT_CONTEXT:END -->

## Runtime Map

<!-- GENERATED:RUNTIME_MAP:START -->
- Server: DeepThree (167.253.158.103)
- SSH: 100.113.246.119:22 (Tailscale only), non-root `banirisset`
- Gateway Service: `hermes-gateway.service`
- Config Home: `/home/hermes/.hermes`
- Repository: `ariwarli/hermes-agent` (forked from NousResearch)
- Environment Variables: `/home/hermes/.hermes/.env`
...
- Proton Pass CLI installed via Homebrew.
- Helper script: `ops/scripts/proton_pass_helper.py`
- Status: CLI ready, awaiting interactive login (`pass-cli login`).
...
1. Tentukan apakah perubahan itu durable (relevan untuk chat berikutnya).
2. Pilih blok yang tepat: `GENERATED:CURRENT_CONTEXT`, `GENERATED:RUNTIME_MAP`, `GENERATED:TELEGRAM_MAP`, `GENERATED:INCIDENTS`.
3. Rewrite blok terkait saja.
4. Ringkas hasil.
5. Jaga kebersihan (hapus state lama yang superseded).
