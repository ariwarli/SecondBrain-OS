#!/usr/bin/env python3
"""
REED Automation — Model Health Checker
...
BASELINE_MODEL = "maiarouter/maia/liquid/lfm-2.5-1.2b-thinking"
...
def get_current_model():
    ...
    for key, sess in sessions.items():
        ...
        # Track models used
        model = sess.get("model", {}).get("name") or sess.get("model", {}).get("id", "")
        if model:
...
def check_gateway_health():
    ...
    try:
        result = os.popen("systemctl is-active openclaw-gateway.service 2>/dev/null").read().strip()
