...
File ini adalah **aturan utama** bagaimana REED dan automation layer-nya memproses, mengklasifikasi, dan mengarsipkan semua yang masuk ke Inbox.
...
**Action:** Kalau konten sudah siap → automation REED auto-schedule ke content topic Telegram untuk nagger.
...
**Action:** Kalau ada follow-up yang overdue → automation REED kirim reminder di topic `personal-crm`.
...
- Output reed-researcher → forward ke Telegram topic `Research` (topic ID: TBD — update setelah dibuat)
- Kalau butuh web search → spawn reed-researcher

Action: REED delegate ke reed-researcher, output dikirim ke topic Research.
...
**Aturan:** Kalau user mention nama project → REED langsung tau folder tujuannya. Kalau nggak jelas → masuk ke `inbox/unsorted/` untuk diklasifikasi manual nanti.
...
Klasifikasi (5 bucket)
        ...
        └── 🔍 Research (aktif) → Research topic (TBD) + reed-researcher
        │
        ▼
        │
        ▼
Automation REED (kalau perlu):
...
2. Reply di Inbox: `"Gw masukin ini ke unsorted. Lo mau gw rute ke mana? (Project/Content/CRM/Task/Knowledge)"`
3. Kalau user jawab → REED route ulang + hapus dari unsorted

---

## Daily Cleanup

Setiap jam 07:00, automation REED:
1. Cek `inbox/unsorted/` — ada yang pending?
2. Cek apakah semua inbox item hari sebelumnya udah di-route
3. Report ke topic `updates`: `"Inbox clean: X items routed, Y unsorted"`
...
- **Reply confirmation.** REED dilarang mengirim konfirmasi routing di Inbox. REED harus langsung memberikan respon atau eksekusi di topik tujuan. User harus tau "udah gw handle".
