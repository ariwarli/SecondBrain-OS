...
INPUT_HTML = Path(__file__).parent / "2026-03-29-sibuk-vs-growth.html"
OUTPUT_DIR = Path(__file__).parent / "slides-sibuk-vs-growth"
...
async def export_slides():
    async with async_playwright() as p:
        ...
        # Hide IG chrome, strip frame to bare slide viewport
        await page.evaluate("""() => {
        ...
        for i in range(TOTAL_SLIDES):
            await page.evaluate(
                ...
                f"document.querySelector('.carousel-track').style.transform='translateX({-i * VIEW_W}px)';"
            )
            await page.wait_for_timeout(500)
